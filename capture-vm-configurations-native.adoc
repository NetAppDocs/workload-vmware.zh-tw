...
+
請參閱「命令列參數」章節。 https://www.dell.com/en-us/shop/vmware/sl/rvtools["RVTools 文件"^] 詳情請見下文。
...
+
規劃中心已顯示。在規劃中心，您可以上傳虛擬機器清單數據，查看先前的清單資料上傳記錄，並使用任何虛擬機器清單資料集規劃遷移，然後選擇「遷移到 AWS 原生運算」。顯示 VMware 遷移顧問啟動畫面。
...
. 選擇*保存*。
. 選取 * 複製 API 存取權杖 * 。
. 儲存此令牌以準備與 Workload Factory 遷移顧問一起使用。
