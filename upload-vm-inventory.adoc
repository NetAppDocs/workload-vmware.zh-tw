...
+
規劃中心已顯示。在規劃中心，您可以上傳虛擬機器清單數據，查看先前的清單資料上傳，並使用任何虛擬機器清單資料集規劃遷移。
...
在 https://www.dell.com/en-us/shop/vmware/sl/rvtools["深入瞭解 RVTools"^]
...
. 輸入您將建立的 VM 清單資料集的名稱。
. 從以下位置下載並安裝 RVTools 4.4.2 或更高版本： https://www.dell.com/en-us/shop/vmware/sl/rvtools["RVTools 網站"^]
...
+
請參閱「命令列參數」章節。 https://www.dell.com/en-us/shop/vmware/sl/rvtools["RVTools 文件位於「資源」部分"^] 詳情請見下文。
...
* link:launch-migration-advisor-gcve.html["建立 Google Cloud VMware Engine 部署計劃"]
