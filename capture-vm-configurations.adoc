...
.步驟
. 登入 Workload Factory 。
. 在 VMware 方塊中、選取 * 規劃與部署 * 。
+
image:screenshot-choose-method.png["您可以用來收集目前 VM 組態資訊的各種方法的螢幕擷取畫面。"]
...
+
如需詳細資訊、請參閱中的「命令列參數」一章 https://resources.robware.net/resources/prod/RVTools.pdf["RVTools 文件"^] 。
...
====
.接下來呢？
link:launch-onboarding-advisor.html["使用移轉顧問建立部署計畫"]。
